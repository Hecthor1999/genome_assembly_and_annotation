...
# Load the annotation
annotation <- fread("/data/users/harribas/assembly_course/annotation/output/final/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
bed_genes <- annotation %>%
    ...
    select(-V9)
# use the assembly.fasta.fai file from the flye assembly
top20_scaff <- fread("/data/users/harribas/assembly_course/assembly/results_fly/assembly.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
